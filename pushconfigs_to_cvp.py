...
CONFIGLET_PREFIX = 'ADC_CVP_DEMO_'   # Configlet name Prefix
CONFIGLET_FILENAME_EXT = '.conf'    # filenames that end with .conf
...
def createconfiglets():
    ...
    for r,d,f in os.walk(CONFIGLET_SUBDIR):
        for filename in f:
            if CONFIGLET_FILENAME_EXT in filename:  # this is a configlet file 
    
                configlet_name = CONFIGLET_PREFIX + ''.join(filename.split())[:-5]
